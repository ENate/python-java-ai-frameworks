...
public class SpiderLeg {
	...
	// web browser.
	Logger logging = LoggerFactory.getLogger(SpiderLeg.class);
	private static final String USER_AGENT = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/535.1 (KHTML, like Gecko) Chrome/13.0.782.112 Safari/535.1";
	private List<String> links = new LinkedList<>();
	private Document htmlDocumentOld;
	...
	public boolean crawl(String url) {
		try {
			...
			this.htmlDocumentOld = htmlDocument;
			...
			if (!connection.response().contentType().contains("text/html")) {
				logging.info("**Failure** Retrieved something other than HTML");
				return false;
			}
			Elements linksOnPage = htmlDocument.select("a[href]");
			logging.info("Found ({} ) links", linksOnPage.size());
	...
	public boolean searchForWord(String searchWord) {
		...
		if (this.htmlDocumentOld == null) {
			logging.info("ERROR! Call crawl() before performing analysis on the document");
			return false;
		}
		logging.info("Searching for the word {} ...", searchWord);
		String bodyText = this.htmlDocumentOld.body().text();
